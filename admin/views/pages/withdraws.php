...
$where = []; $params = [];
if ($uid !== '')  { $where[] = 'w.user_id IN (SELECT id FROM bot_users WHERE uid LIKE :uid)'; $params[':uid'] = "%{$uid}%"; }
if ($date !== '') { $where[] = 'DATE(w.created_at) = :date'; $params[':date'] = $date; }
$whereSQL = $where ? 'WHERE ' . implode(' AND ', $where) : '';

$total = (int)(Db::fetch("SELECT COUNT(*) as c FROM bot_withdraws w {$whereSQL}", $params)['c'] ?? 0);
$totalPages = max(1, (int)ceil($total / $limit));
$rows = Db::fetchAll(
    "SELECT w.*, u.uid as user_uid, u.nickname
     FROM bot_withdraws w LEFT JOIN bot_users u ON w.user_id = u.id
     {$whereSQL} ORDER BY w.id DESC LIMIT {$limit} OFFSET {$offset}",
    $params
);
$totalAmount = (float)(Db::fetch(
    "SELECT COALESCE(SUM(w.amount),0) as s FROM bot_withdraws w {$whereSQL}", $params
)['s'] ?? 0);

$base = '/index.php?page=withdraws' . ($uid?'&uid='.urlencode($uid):'') . ($date?"&date={$date}":'');
?>

<form class="flex gap-3 items-center" style="margin-bottom:20px;flex-wrap:wrap">
    <input type="hidden" name="page" value="withdraws">
    ...
    <?php if ($uid || $date): ?><a href="/index.php?page=withdraws" class="btn btn-secondary">重置</a><?php endif; ?>
    <span class="text-sm" style="margin-left:auto">
        共 <?= number_format($total) ?> 条 &nbsp;|&nbsp;
        <strong class="text-red">总计 <?= Helper::money($totalAmount) ?> 元</strong>
    </span>
</form>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="text-right"><span class="text-red" style="font-weight:600">-<?= Helper::money($row['amount']) ?></span></td>
